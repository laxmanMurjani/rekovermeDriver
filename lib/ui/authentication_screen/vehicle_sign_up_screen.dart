  ...
  Widget titleAndFormWidget(cont) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          titleWidget(widget.isTow
              ? "Vehicle_Registration".tr
              : 'Road Side Serviceman'),

          widget.isTow
              ? serviceTypeDropdown(cont)
              : licensePlateTextField(cont),

          widget.isTow ? truckMakeTextField(cont) : SizedBox(),
          //insuranceTextField(cont)

          widget.isTow
              ? licensePlateNumberTextField(cont)
              : selectServicesWidget(cont),
  ...
  Widget titleWidget(title) {
    return Padding(
      ...
      child: Text(
        title,
        style: TextStyle(
            fontWeight: FontWeight.w400, fontSize: 24.sp, color: Colors.black),
      ),
    );
  }
    ...
    return Container(
        ...
        child: Padding(
          ...
          child: DropdownButton<ServiceType>(
            ...
            items: cont.serviceTypeList
                .map<DropdownMenuItem<ServiceType>>((location) {
          ...
          ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                return Row(
                  ...
                  children: [
                    Text(
                      '${cont.serviceTypeList[index].name}',
                      style:
                          TextStyle(fontSize: 15, fontWeight: FontWeight.w400),
                    ),
                    Checkbox(
                      value: selectedServicesList
                          .contains(cont.serviceTypeList[index].id),
                      onChanged: (bool? value) {
                        setState(() {
                          value == true
                              ? selectedServicesList.add(val)
                              : selectedServicesList.remove(val);
                        });
                      },
                    ),
                  ],
  ...
  Widget createAccountButton(cont) {
    return InkWell(
      onTap: () {
        ...
        widget.isTow
            ? cont.registerUser()
            : cont.registerRoadSideDriver(selectedServicesList);
      },
      child: Container(
