...
import 'package:mozlit_driver/ui/authentication_screen/otp_screen_for_registration.dart';
...
class UserController extends BaseController {
  ...
  Future<void> registerUser() async {
    ...
    try {
      ...
      params["service_type"] = "${serviceType?.id ?? 1}";
  ...
  Future<void> verifyOTpForRegistration(String otp, mobile) async {
    ...
    try {
      ...
      await apiService.postRequest(
          url: ApiUrl.verifyOTPForRegistration,
          params: params,
          onSuccess: (Map<String, dynamic> data) {
            //dismissLoader();
            //_userController.userToken.value.accessToken
            // userToken.value =
            //     loginResponseModelFromJson(jsonEncode(data["response"]));
    ...
    try {
      ...
      await apiService.getRequest(
          ...
          onError: (ErrorType errorType, String? msg) {
            ...
            userToken.value = LoginResponseModel();
            userToken.refresh();
            Get.offAll(() => LoginOtpScreen(), predicate: (_) => true);
          });
    } catch (e) {
      log("message ===>   $e");
      dismissLoader();
